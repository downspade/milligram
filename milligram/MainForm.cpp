...
LRESULT CMainForm::ProcessMessages(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, bool &CallDefault)
{
	switch (message)
	{
		...
		case WM_CONTEXTMENU:
			// NOTE:Lock 中に右クリックでメニューを出してリストを表示→もう一度右クリックでメニューを出して画像を表示とするとこれが呼ばれる
			ShowContextMenu();
			break;
...
bool CMainForm::GetImageList(std::wstring Src, std::vector<CImageInfo> &Dest, bool SubFolder, bool EnableFileMask, std::wstring FileMaskString) // c:\windows\dest\*.dll のような形式で Src は指定する
{
...
{
	ToggleShowList(Mode, 1); // Mode 0:反転 1:リスト表示 2:画像表示 3:強制画像表示
}

void CMainForm::ToggleShowList(EShowMode Mode, int Dir) // Mode 0:反転 1:リスト表示 2:画像表示 3:強制画像表示
{
